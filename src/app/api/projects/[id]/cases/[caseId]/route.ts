...
export async function PATCH(req: NextRequest, { params }: Params) {
  ...
  const schema = z.object({
    ...
    type: z.enum(['E2E', 'INTEGRATION', 'API', 'UI', 'MANUAL']).optional(),
    status: z.enum(['PASSED', 'FAILED', 'UNTESTED', 'IN_PROGRESS']).optional(),
  ...
  try {
    ...
    let nextFolderId: string | undefined
    ...
    const testCase = await prisma.testCase.update({
      ...
      data: {
        ...data,
        ...(typeof nextFolderId !== 'undefined' && { folderId: nextFolderId }),
        ...(steps && {
          steps: {
            deleteMany: { testCaseId: caseId },
            create: steps.map(s => ({ order: s.order, action: s.action })),
          },
        }),
      },
      select: {
